...
/**
 * The Class ServerThreadCommunication. The object of this class is created
 * every time new client connects to the server. For every client there is
 * personal thread in server
 */
		...
		try {
			...
			while (true) {
				...
				if (!aliveThreads.contains(this)) {
					ServerStart.loggerServer.info(new String("Client " + login + " is disconnected from a server"));
					break;
				}

				if (request.getRequest() == Request.Requests.EXIT) {
					if (aliveThreads.remove(this)) {
						ServerStart.loggerServer.info(new String("Client " + login + " finished its work"));
					}
					break;
				}
